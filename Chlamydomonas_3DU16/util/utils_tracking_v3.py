...
peaks_with_intensities = peaks_with_intensities[:, [2, 1, 0, 3]]
'''
...
class Detect2Tracking:
    ...
    def custom_peak_local_max_sort(
        ...
        if use_gpu:
            xp = cp
            # ** UserWarning: ignoring size because footprint is set **
            # _local_max = maximum_filter_gpu(img, size=2 * min_dist + 1, footprint=footprint)
            _local_max = maximum_filter_gpu(img, footprint=footprint) 
        else:
            xp = np
            # _local_max = maximum_filter_cpu(img, size=2 * min_dist + 1, footprint=footprint)
            _local_max = maximum_filter_cpu(img, footprint=footprint)
        ...
        # Retrieve the intensity values of the detected peaks.
        intensities = img[_peak_mask]
        print(xp.sum(_peak_mask))  # True の数を出力
        ...
        print(f"valid_idx.intensities: {intensities[valid_idx]}")
        ...
        # Extract intensities based on valid_idx
        valid_intensities = intensities[valid_idx]
        
        # Sort intensities in descending order, and get the corresponding indices
        sorted_idx = xp.argsort(valid_intensities)[::-1]
        print(f"sorted_idx: {sorted_idx}")
        print(f"sorted_idx.intensities: {valid_intensities[sorted_idx]}")

        # Select the top `detect_num` peaks based on the sorted intensities.
        selected_idx = sorted_idx[:min(detect_num, len(sorted_idx))]

        # Extract the coordinates and intensities of the selected peaks.
        sorted_peaks = peaks[valid_idx][selected_idx]
        sorted_intensities = valid_intensities[selected_idx]
    ...
    def tracking_using_intensity(
        ...
        min_diff_indices = []
        for intensity_diff, cropped_img in zip(
            intensity_diffs, smoothed_cropped_images
        ):
            ...
            if below_threshold.size > 0:
                ...
                # use max intensity instead of min intensity
                max_intensity_idx = np.unravel_index(
                    np.argmax(cropped_img, axis=None), cropped_img.shape
                )
                min_diff_indices.append(np.array(max_intensity_idx))
        ...
        tracked_coordinates_with_intensity = [
            ...
            for (z_start, y_start, x_start), min_diff_idx, cropped_img in zip(
                zip(crop_z_start, crop_y_start, crop_x_start),
                min_diff_indices,
                smoothed_cropped_images,
            )
        ]
    ...
    def initialize_kalman_filters(self, initial_coordinates, indices=None):
        if indices is None:
            indices = range(len(initial_coordinates))
        for idx, coord in zip(indices, initial_coordinates):
            kf = KalmanFilter(dim_x=4, dim_z=4)
            kf.x = np.zeros((4, 1))
            kf.x[0:3] = coord[:3].reshape(-1, 1)  # x, y, z を初期化
            kf.x[3] = coord[3]  # intensity を初期化
            ...
            # プロセスノイズ共分散行列 Q の設定
            qx = 10.0  # X方向のプロセスノイズの分散
            qy = 10.0  # Y方向のプロセスノイズの分散
            qz = 200  # Z方向のプロセスノイズの分散（大きめに設定）
            q_intensity = 50.0  # intensity のプロセスノイズの分散

            kf.Q = np.diag([qx, qy, qz, q_intensity])

            # 観測ノイズ共分散行列 R の設定
            rx = 1  # X方向の観測ノイズの分散
            ry = 1  # Y方向の観測ノイズの分散
            rz = 20  # Z方向の観測ノイズの分散（大きめに設定）
            r_intensity = 100  # intensity の観測ノイズの分散

            kf.R = np.diag([rx, ry, rz, r_intensity])
    ...
    def calculate_cost_matrix(self, predicted_positions, observed_positions):
        num_predictions = len(predicted_positions)
        num_observations = len(observed_positions)
        cost_matrix = np.zeros((num_predictions, num_observations))

        for i, pred in enumerate(predicted_positions):
            for j, obs in enumerate(observed_positions):
                # 各方向の差分を計算
                dx = pred[0] - obs[0]
                dy = pred[1] - obs[1]
                dz = pred[2] - obs[2]
                ...
                # Z方向の重みを減らして距離を計算
                weight_z = 0.1  # Z方向の重み（調整可能）
                distance = np.sqrt(dx**2 + dy**2 + (weight_z * dz) ** 2)

                cost_matrix[i, j] = distance
        return cost_matrix

    def calculate_cost_matrix_xy(self, predicted_positions, observed_positions):
        num_predictions = len(predicted_positions)
        num_observations = len(observed_positions)
        cost_matrix = np.zeros((num_predictions, num_observations))
    ...
    def match_detections(self, cost_matrix, gating_threshold):
    ...
    def track_and_update(
        ...
        # 予測ステップ
        predicted_positions = []
        filter_indices = list(self.filters.keys())
        for idx in filter_indices:
            kf = self.filters[idx]
            kf.predict()
            predicted_state = kf.x.flatten()
            predicted_positions.append(predicted_state)
            # print(f"Filter {idx} predicted state:\n{predicted_state}")  # デバッグ用

        # 観測ステップ
        predicted_positions_np = np.array(predicted_positions)
        predicted_positions_tracking = predicted_positions_np[:, [2, 1, 0, 3]]
        tracked_coordinates_with_intensity, peak_record = self.tracking_using_intensity(
            img=img,
            z_around=z_around,
            y_around=y_around,
            x_around=x_around,
            pre_coordinate_with_intensity=predicted_positions_tracking.tolist(),
            peak_record_with_intensity=peak_record_with_intensity,
            threshold=threshold,
            sigma=sigma,
            do_profile=do_profile,
        )
        tracked_coordinates_with_intensity_np = np.array(tracked_coordinates_with_intensity)
        tracked_coordinates_with_intensity_kf = tracked_coordinates_with_intensity_np[:, [2, 1, 0, 3]]
        tracked_coordinates_with_intensity_kf = tracked_coordinates_with_intensity_kf.tolist()
        self.observed_points.append(tracked_coordinates_with_intensity_kf)

        # 予測位置と観測位置をNumPy配列に変換
        predicted_positions = np.array(predicted_positions)
        observed_positions = np.array(tracked_coordinates_with_intensity_kf)

        # 距離行列の作成（XY平面の距離のみを使用）
        cost_matrix = self.calculate_cost_matrix_xy(
            predicted_positions, observed_positions
        )

        # マッチングの実行
        gating_threshold = 10.0  # XY平面でのゲーティング閾値（適切に調整してください）
        matches = self.match_detections(cost_matrix, gating_threshold)

        # マッチしたフィルターを更新
        matched_filter_indices = []
        matched_observation_indices = []
        for r, c in matches:
            idx = filter_indices[r]
            kf = self.filters[idx]
            measurement = observed_positions[c][:4].reshape(-1, 1)

            # XY平面でのゲーティングを適用
            dx = predicted_positions[r][0] - measurement[0]
            dy = predicted_positions[r][1] - measurement[1]
            distance_xy = np.sqrt(dx**2 + dy**2)

            if distance_xy < gating_threshold:
                prev_position = kf.x[:3].copy()  # 更新前の位置を保存

                kf.update(measurement)

                # 更新後の位置を取得
                updated_position = kf.x[:3]

                # XY平面での移動距離を計算
                dx_move = updated_position[0] - prev_position[0]
                dy_move = updated_position[1] - prev_position[1]
                movement_xy = np.sqrt(dx_move**2 + dy_move**2)

                # 移動距離が閾値を超える場合、移動を制限
                max_movement_xy = 5.0  # 許容する最大移動距離（適切に調整してください）
                if movement_xy > max_movement_xy:
                    # 移動量をスケーリングして制限
                    scaling_factor = max_movement_xy / movement_xy
                    kf.x[0] = prev_position[0] + dx_move * scaling_factor
                    kf.x[1] = prev_position[1] + dy_move * scaling_factor
                    # print(f"Filter {idx}: XY movement limited to {max_movement_xy}")

                # print(
                #     f"Filter {idx} updated with observation {c}:\n{kf.x.flatten()}"
                # )  # デバッグ用
                matched_filter_indices.append(idx)
                matched_observation_indices.append(c)
            else:
                print(
                    f"Filter {idx}: Measurement rejected due to large XY distance ({distance_xy})"
                )

        # マッチしなかったフィルターを処理（例：削除）
        unmatched_filter_indices = set(filter_indices) - set(matched_filter_indices)
        filters_to_delete = []
        for idx in unmatched_filter_indices:
            # print(f"Filter {idx} deleted due to no matching observation")
            filters_to_delete.append(idx)
        # フィルターを削除
        for idx in filters_to_delete:
            del self.filters[idx]

        # マッチしなかった観測値に対して新しいフィルターを作成
        unmatched_observation_indices = set(range(len(observed_positions))) - set(
            matched_observation_indices
        )
        for c in unmatched_observation_indices:
            new_idx = (
                max(self.filters.keys(), default=-1) + 1
            )  # 新しいインデックスを割り当て
            coord = observed_positions[c]
            # 新しいフィルターを初期化
            self.initialize_kalman_filters([coord], indices=[new_idx])
            # print(f"Created new filter {new_idx} for observation {c}")
    ...
    def modify_tracking_result_with_centroid(
        self,
        img,
        z_around,
        y_around,
        ...
        """_summary_
        n = 3, ROIsizs=8,20,20
        1 ROI: 3.1, 3.6, 4.6, 5.1, 4.4, 4.9 [ms]
        2 ROI: 9.1, 8.8, 8.8, 9.1, 8.8, 8.8 [ms]
        3 ROI: 13.1, 11.3, 10.5, 11.1, 11.6, 13.2 [ms]
        """
        ...
        # Extract the shape of the image for boundary checks.
        shapez, shapey, shapex = img.shape

        # Unpack the previous coordinates and their intensities.
        pz, py, px, pre_intensity = np.array(pre_coordinate_with_intensity).T
        if do_profile:
            print(f"coords: {pz, py, px, pre_intensity}")
        ...
        for cropped_idx, (z_start, z_end, y_start, y_end, x_start, x_end) in enumerate(
            zip(crop_z_start, crop_z_end, crop_y_start, crop_y_end, crop_x_start, crop_x_end)
        ):
            cropped_img = img[z_start:z_end, y_start:y_end, x_start:x_end]
            threshold = np.max(cropped_img) / divided_num
            if do_profile:
                print(f"threshold: {threshold}")
            # threshold = np.max(cropped_img) / 6
            
            # >> 3.1. ラベルを作成
            if use_watershed:
                # Step 1: スムージング
                # cropped_img = gaussian_filter(cropped_img, sigma=0.7)
                # Step 2: 距離変換
                distance = ndimage.distance_transform_edt(cropped_img>threshold)
                # Step 3: 局所最大をラベルとして使用
                markers = measure.label(distance > ditance_num)
                # Step 4: Watershed
                labeled_image = watershed(-distance, markers=markers, mask=cropped_img>threshold)
            else:
                # Step 1: スムージング
                # cropped_img = gaussian_filter(cropped_img, sigma=0.7)
                # Step 2: 二値化処理：輝度のしきい値を設定し、しきい値以上を1、それ以外を0にする
                binary_image = cropped_img>threshold
                # Step 3: ラベリング処理：二値化した画像内の塊（連結成分）を識別
                labeled_image, num_features = ndimage.label(binary_image)
                
            # 重心計算
            properties = measure.regionprops(labeled_image, intensity_image=cropped_img)
            
            # 現在の座標
            z, y, x, intensity = pre_coordinate_with_intensity[cropped_idx]

            # 元の座標がラベルに含まれるかを確認
            local_z, local_y, local_x = int(z - z_start), int(y - y_start), int(x - x_start)
            if (
                0 <= local_z < labeled_image.shape[0]
                and 0 <= local_y < labeled_image.shape[1]
                and 0 <= local_x < labeled_image.shape[2]
            ):
                # print(labeled_image.shape)
                # print(local_z, local_y, local_x)
                label = labeled_image[local_z, local_y, local_x]
                if label > 0:
                    # ラベルに属している場合、重心を取得
                    ...
                    updated_coordinates.append(
                        (
                            int(centroid[0] + z_start),
                            int(centroid[1] + y_start),
                            int(centroid[2] + x_start),
                            cropped_img[int(centroid[0]), int(centroid[1]), int(centroid[2])],
                        )
                    )
                else:
                    # 属していない場合、元の座標を保持
                    updated_coordinates.append((z, y, x, intensity))
            else:
                # ラベルが範囲外の場合、元の座標を保持
                updated_coordinates.append((z, y, x, intensity))
        ...
        # >> 5. 追跡座標とピーク情報を更新（if there is updatetion）
        # # next2の置換
        # next2 = np.asarray(updated_coordinates)
        # peaksの置換
        for peak_num in range(len(updated_coordinates)):
            centroid = updated_coordinates[peak_num]
            # ラベルから輝度情報を取得
            z_centoroid, y_centoroid, x_centoroid, centroid_intensity = centroid
            # 重心を近傍探索結果の参照ピクセルとして置き換える
            peak_record_with_intensity[-1][peak_num] = [0., float(centroid[0]), float(centroid[1]), float(centroid[2]), float(centroid_intensity)]

        if do_profile:
            print(f"Total time: {time.perf_counter() - start} s")
            print("------------------")

        return updated_coordinates, peak_record_with_intensity

    def modify_tracking_result_with_centroid_threading(
        self,
        img,
        z_around,
        y_around,
        ...
        """Tracking result modification using centroids."""
        ...
        # Helper function for processing a single cropped region
        def process_cropped_region(args):
            idx, z_start, z_end, y_start, y_end, x_start, x_end = args
            cropped_img = img[z_start:z_end, y_start:y_end, x_start:x_end]
            ...
            if use_watershed:
                ...
                markers = measure.label(distance > ditance_num)
                labeled_image = watershed(-distance, markers=markers, mask=binary_image)
            else:
                labeled_image, _ = ndimage.label(binary_image)

            properties = measure.regionprops(labeled_image, intensity_image=cropped_img)

            z, y, x, intensity = pre_coordinate_with_intensity[idx]
            local_z, local_y, local_x = int(z - z_start), int(y - y_start), int(x - x_start)
            ...
            if (
                ...
                if label > 0:
                    ...
                    return (
                        ...
                        int(centroid[2] + x_start),
                        cropped_img[
                            int(centroid[0]), int(centroid[1]), int(centroid[2])
                        ],
                    )
            return idx, z, y, x, intensity

        # Prepare arguments for multithreading
        args_list = [
            (i, z_start, z_end, y_start, y_end, x_start, x_end)
            for i, (z_start, z_end, y_start, y_end, x_start, x_end) in enumerate(
                zip(crop_z_start, crop_z_end, crop_y_start, crop_y_end, crop_x_start, crop_x_end)
            )
        ]
        ...
        with ThreadPoolExecutor() as executor:
            results = list(executor.map(process_cropped_region, args_list))

        # Sort results by ID to maintain order
        results.sort(key=lambda x: x[0])

        # Extract updated coordinates and replace peak records
        updated_coordinates = [res[1:] for res in results]
        for peak_num, centroid in enumerate(updated_coordinates):
            z_centroid, y_centroid, x_centroid, centroid_intensity = centroid
            peak_record_with_intensity[-1][peak_num] = [
                0.0,
                float(z_centroid),
                float(y_centroid),
                float(x_centroid),
    ...
    def modify_tracking_result_with_centroid_threading_threshold_percentage(
        self,
        img,
        z_around,
        y_around,
        ...
        def process_cropped_region(args):
            ...
            # threshold_cp = np.percentile(img, 98)  # 上位2%の輝度を閾値に
            # threshold = cp.asnumpy(threshold_cp)
            threshold = np.max(cropped_img) / 6
            ...
            z, y, x, intensity = pre_coordinate_with_intensity[idx]
            local_z, local_y, local_x = z - z_start, y - y_start, x - x_start
        ...
        # with ProcessPoolExecutor(max_workers=4) as executor:
            results = list(executor.map(process_cropped_region, args_list))
        # print(f"Process time: {time.time() - s_time}")
        # Sort results by ID to maintain order
        results.sort(key=lambda x: x[0])
