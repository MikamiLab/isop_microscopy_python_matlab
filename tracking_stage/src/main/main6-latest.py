...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        pushButton_11.pressed.connect(self.Apply)
    ...
    def track(self):
        ...
        LogThread(self).start()
        # RecThread(self).start()

        self.save_thread = RecThread()
    ...
    def trackoff(self):
        ...
        LogThread(self).log_file.close()
        LogThread(self).quit()
        # RecThread(self).writer.release()
        # RecThread(self).quit()

        if self.save_thread:
            self.save_thread.writer.release()
    ...
    def valuechange6(self):
        global low_speed
        low_speed = self.sp6.value()
    def valuechange7(self):
        global top_speed
        top_speed = self.sp7.value()
    def valuechange8(self):
        global acce_time
        acce_time = self.sp8.value()
    ...
    # def start_Rec_save(self):
    #     self.save_thread = RecThread()
    #     self.save_thread.start()

    # def stop_Rec_save(self):
    #     if self.save_thread:
    #         self.save_thread.quit()
    #         self.save_thread.wait()
...
class RecThread(QThread):
    def __init__(self):
        ...
        try:
            ...
            pass
        self.filename = variable.now.strftime(os.getcwd()+"/log/"+"%Y%m%d%H%M%S/")+"rec.avi"
        variable.video_path = self.filename
        self.fourcc = cv2.VideoWriter_fourcc(*'MJPG')
        self.writer = cv2.VideoWriter(self.filename, self.fourcc, fps, size)
    
    def run(self):
        self.recthread = CapThread()
        self.recthread.frame_signal.connect(self.save_frame)
...
class StageThread(QThread):
    ...
    def run(self):
        self.stagethread = CapThread

    @pyqtSlot(object)
    def coord(self,frame_num):
        ser.write(b"Q:\r\n")
...
class CapThread(QtCore.QThread):
    ...
    stage_signal = pyqtSignal(object)
    ...
    def run(self):
        ...
        now = datetime.datetime.now()
        self.now = now
        self.filename = variable.video_path
        fourcc = cv2.VideoWriter_fourcc(*'MJPG')
        ...
        while end_flag:
            ...
            if (flag == True) :
                try:
                    self.frame_signal.emit(self.frame)
                    self.stage_signal.emit(self.frame_num)
                    #RecThread().writer.write(self.frame) 
                except:
                    pass
            ...
            if (flag == True) or (track_flag == True) :
                ...
                #log_file.write("frame,time,x(pixel),y(pixel),radius(pixel),asobi(pixel),Δt(s),Δd(μm),track_v(μm/s),err(μm),sum_x(pixel),sum_y(pixel)\n")
                LogThread().log_file.write(str(self.frame_num)+","+str(time_delta.total_seconds())+","+str(detection_x)+","+str(detection_y)+","
                    +str(self.radius)+","+str(asobi)+","+str(t)+","+str(d)+","+str(Err)+","+str(variable.sdetx)+","
                    +str(variable.sdety)+"\n")   
