...
# 各行の点から点への線分の長さを計算する関数
def calculate_segment_lengths(points1, points2, threshold=10.0):
    num_points = points1.shape[0]
    segment_lengths = np.zeros((num_points,))
    
    for i in range(num_points):
        segment_lengths[i] = np.linalg.norm(points2[i] - points1[i])
        
    indices_above = np.where(segment_lengths > threshold)[0]
